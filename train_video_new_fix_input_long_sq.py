...
class DeepFakeDataModule(pytorch_lightning.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        val_dataset = deepfake_video(
            data_path=os.path.join(self._DATA_PATH, "val_mini.csv"),
            clip_sampler=pytorchvideo.data.make_clip_sampler("uniform", self._CLIP_DURATION, None, True),
    ...
    def train_dataloader(self):
        ...
        train_dataset = deepfake_video(
            ...
            clip_sampler=pytorchvideo.data.make_clip_sampler("random", self._CLIP_DURATION + 4 * self._CLIP_DURATION / T),
    ...
    def test_dataloader(self):
        ...
        return torch.utils.data.DataLoader(
            ...
            num_workers=self._NUM_WORKERS * 2,
        )
...
def make_deepfake_vitb():
    ...
    return get_deepfake_av_model(256, 128, 32)
    

class VideoClassificationLightningModule(pytorch_lightning.LightningModule):
    def __init__(self, tta=False, use_pretrained_model=True):
        super().__init__()
...
def train():
    ...
    for t in [32, 64]:
        ...
        T = t
        name = f"AV_ViTB_T{T}"
        tb_logger = pl_loggers.TensorBoardLogger(save_dir="logs/", name=name, version="aug")
        csv_logger = pl_loggers.CSVLogger(save_dir="logs/", name=name, version="aug")
        ...
        # def convert_sq_16to32(path):
        #     ck = torch.load(path)
        #     pos_emb_temp = ck["state_dict"]["av_model.cls_positional_encoding.pos_embed_temporal"]
        #     pos_emb_temp2 = ck["state_dict"]["av_model.cls_positional_encoding2.pos_embed_temporal"]
        #     ck["state_dict"]["av_model.cls_positional_encoding.pos_embed_temporal"] = torch.repeat_interleave(pos_emb_temp, T // 16, dim=1)
        #     ck["state_dict"]["av_model.cls_positional_encoding2.pos_embed_temporal"] = torch.repeat_interleave(pos_emb_temp2, T // 16, dim=1)
        #     torch.save(ck, path.replace(".ckpt", f"_convert_T={T}.ckpt"))
            
        # convert_sq_16to32("logs/AV_ViTB/version_76/checkpoints/last.ckpt")
        
        # classification_module = VideoClassificationLightningModule.load_from_checkpoint(f"logs/AV_ViTB/version_116/checkpoints/last.ckpt", strict=False)
        trainer = pytorch_lightning.Trainer(callbacks=[checkpoint_callback], logger=[tb_logger, csv_logger], max_epochs=5, precision="16-mixed", val_check_interval=1/4)
...
def test(tta=False):
    ...
    trainer.test(classification_module, data_module, ckpt_path="/nasdata2/private/zwlu/classify/Kaggle/deepfake_adv/logs/AV_ViTB/version_118/checkpoints/epoch=2-step=45374.ckpt")


class VideoPredictor():
    
    def __init__(self, checkpoint, decode_audio: bool = True,
        decoder: str = "pyav", transforms = None, clip_duration=4, target_batch=4, fp_16=True) -> None:
...
if __name__ == "__main__":
    ...
    train()
    # test(tta=False)
    # predictor = VideoPredictor("/nasdata2/private/zwlu/classify/Kaggle/deepfake_adv/logs/AV_ViTB/version_118/checkpoints/epoch=3-step=52937.ckpt")
    # videos = glob.glob("/nasdata2/private/zwlu/classify/Kaggle/deepfake_adv/data/video/phase2/testset1seen/*")
    ...
    #     res = predictor(i)
    # # res = predictor("/nasdata2/private/lzhao/workspace/kaggle/DeepfakeVideo/dataset/video/phase1/valset/d23a91c601d674448b5a2986216d05f1.mp4")
    # # res = predictor("/nasdata2/private/lzhao/workspace/kaggle/DeepfakeVideo/dataset/video/phase1/valset/d23a91c601d674448b5a2986216d05f1.mp4")
    # # res = predictor("/nasdata2/private/lzhao/workspace/kaggle/DeepfakeVideo/dataset/video/phase1/valset/d23a91c601d674448b5a2986216d05f1.mp4")
    # # res = predictor("/nasdata2/private/lzhao/workspace/kaggle/DeepfakeVideo/dataset/video/phase1/valset/d23a91c601d674448b5a2986216d05f1.mp4")
    ...
    #         pd.DataFrame(submit_csv).to_csv(f"/nasdata2/private/zwlu/classify/Kaggle/deepfake_adv/val_submit_{idx:06}.csv", index=False)
    
    # pd.DataFrame(submit_csv).to_csv(f"/nasdata2/private/zwlu/classify/Kaggle/deepfake_adv/val_submit_final.csv", index=False)
