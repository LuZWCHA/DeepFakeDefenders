...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, zero_init_residual=False):
        ...
        self.inplanes = 64
        self.conv1 = nn.Conv2d(5, 64, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        grey_x = torch.mean(x, dim=1, keepdim=True)
        gradient_x = list(torch.gradient(grey_x, dim=(2, 3)))
        new_NPR = torch.concat([NPR] + gradient_x, dim=1)
        
        x = self.conv1(new_NPR * 2.0 / 3.0)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
